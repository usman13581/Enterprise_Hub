      ...
      provisioned = await this.prisma.$transaction(async (tx) => {
        ...
        const supplier = await tx.supplier.create({
          data: {
            ...
            name: 'Demo Stone Supplier',
            contact: 'Demo purchasing contact',
            phone: input.phone,
            email: `supplier@${slug}.example`,
            address: input.emirate,
        ...
        const customer = await tx.customer.create({
          data: {
            companyId: company.id,
            name: 'Demo Villa Project',
            contact: input.contactName || 'Demo contact',
            phone: input.phone,
            email,
            address: input.emirate,
        ...
        const quotation = await tx.quotation.create({
          data: {
            ...
            number: 'DEMO-QT-0001',
            status: 'approved',
            title: 'Demo villa countertop',
            subtotal,
            vatAmount,
            total,
            purchaseTotal: qty * product.purchasePrice,
            approvedAt: startsAt,
            lines: {
              create: [{
        ...
        const advance = await tx.advancePayment.create({
          data: {
            ...
            customerId: customer.id,
            jobId: job.id,
            number: 'DEMO-ADV-0001',
            amount: 500,
            allocatedAmount: 0,
            method: 'bank_transfer',
            reference: 'DEMO-TRANSFER',
            notes: 'Generated demo advance',
          },
        });
        const invoice = await tx.invoice.create({
          data: {
            ...
            status: 'issued',
            subtotal: subtotal / 2,
            vatAmount: vatAmount / 2,
            total: total / 2,
            purchaseTotal: quotation.purchaseTotal / 2,
            advanceApplied: 0,
            netPayable: total / 2,
            lines: {
              create: [{
