...
export default function SupplierHubPage() {
  ...
  return (
    <section className={page.page}>
      ...
      <p className={page.lede}>{[hub.supplier.email, hub.supplier.phone].filter(Boolean).join(' · ') || 'Supplier hub and cost analytics'}</p>
      ...
      <div className={finance.statGrid}>
        ...
        <Stat title="Catalog purchase" value={money(hub.summary.catalogPurchaseValue)} />
        <Stat title="Quoted cost" value={money(hub.summary.quotedCost)} />
        <Stat title="Estimated quoted margin" value={money(hub.summary.estimatedQuotedMargin)} />
        <Stat title="Payable" value={money(hub.finance.payable)} />
        <Stat title="Overdue" value={money(hub.finance.overdue)} />
        <Stat title="Supplier advances" value={money(hub.finance.advances)} />
      ...
      {hub.products.length ? <TableScroll><table className={finance.table}><thead><tr><th>Product</th><th>SKU</th><th className={finance.numeric}>Purchase</th><th className={finance.numeric}>Sell</th><th>Status</th></tr></thead><tbody>{hub.products.map((product) => <tr key={product.id}><td>{product.name}</td><td>{product.sku || '—'}</td><td className={finance.numeric}>{money(product.purchasePrice)}</td><td className={finance.numeric}>{money(product.sellPrice)}</td><td>{product.active ? 'Active' : 'Inactive'}</td></tr>)}</tbody></table></TableScroll> : <EmptyState>No products are assigned to this supplier.</EmptyState>}
      ...
      {hub.quotationLines.length ? <TableScroll><table className={finance.table}><thead><tr><th>Quotation</th><th>Customer</th><th>Product</th><th className={finance.numeric}>Qty</th><th className={finance.numeric}>Cost</th><th className={finance.numeric}>Margin</th></tr></thead><tbody>{hub.quotationLines.map((line) => <tr key={line.id}><td>{line.quotation.number}</td><td>{line.quotation.customer.name}</td><td>{line.product?.name || '—'}</td><td className={finance.numeric}>{line.qty}</td><td className={finance.numeric}>{money(line.purchasePrice * line.qty)}</td><td className={finance.numeric}>{money((line.sellPrice - line.purchasePrice) * line.qty)}</td></tr>)}</tbody></table></TableScroll> : <EmptyState>No quotation usage recorded.</EmptyState>}
      ...
      {hub.lpos.length || hub.purchaseInvoices.length ? <TableScroll><table className={finance.table}><thead><tr><th>Document</th><th>Type</th><th>Status</th><th className={finance.numeric}>Total</th><th className={finance.numeric}>Balance</th></tr></thead><tbody>{hub.lpos.map((item) => <tr key={item.id}><td>{item.number}</td><td>LPO</td><td>{item.status}</td><td className={finance.numeric}>{money(item.total)}</td><td className={finance.numeric}>—</td></tr>)}{hub.purchaseInvoices.map((item) => <tr key={item.id}><td>{item.number}</td><td>Purchase invoice</td><td>{item.status}</td><td className={finance.numeric}>{money(item.total)}</td><td className={finance.numeric}>{money(item.balance)}</td></tr>)}</tbody></table></TableScroll> : <EmptyState>No purchasing documents recorded.</EmptyState>}
      ...
      {hub.payments.length ? <TableScroll><table className={finance.table}><thead><tr><th>Payment</th><th>Date</th><th>Method</th><th>Reference</th><th className={finance.numeric}>Amount</th><th className={finance.numeric}>Unapplied</th></tr></thead><tbody>{hub.payments.map((payment) => <tr key={payment.id}><td>{payment.number}</td><td>{new Date(payment.paidAt).toLocaleDateString()}</td><td>{payment.method}</td><td>{payment.reference || '—'}</td><td className={finance.numeric}>{money(payment.amount)}</td><td className={finance.numeric}>{money(payment.unappliedAmount)}</td></tr>)}</tbody></table></TableScroll> : <EmptyState>No supplier payments recorded.</EmptyState>}
      ...
      {hub.priceHistory.length ? <TableScroll><table className={finance.table}><thead><tr><th>Date</th><th>Product</th><th className={finance.numeric}>Unit cost</th><th>Source</th></tr></thead><tbody>{hub.priceHistory.map((price) => <tr key={price.id}><td>{new Date(price.effectiveAt).toLocaleDateString()}</td><td>{price.productName}</td><td className={finance.numeric}>{money(price.unitCost)}</td><td>{price.sourceType}</td></tr>)}</tbody></table></TableScroll> : <EmptyState>No posted purchase prices recorded.</EmptyState>}
